...
# -*- coding: utf-8 -*-
"""
DESCRIPTION: Generates the `ICD9_umls2020aa` file, mapping the ICD-9 codes to CUIs, TUIs, and definitions
             querying from **UMLS2020AA**.
...
def main(args):
    ...
    with open(output_file, 'a') as wf:
        for idx, code in enumerate(icd9_codes):
            r = requests.get(f"{BASE_URL}/{code}", params={'ticket': auth_client.get_service_ticket(tgt)})
            ...
            if r.status_code == 200:
                ...
                r = requests.get(
                    ...
                    params={'ticket': auth_client.get_service_ticket(tgt)}
                )
