...
relations_path = os.path.join(snomed_dir, 'snomed_active_relations.txt')
semantic_types_path = os.path.join(snomed_dir, 'snomed_semantic_types.txt')
concepts_path = os.path.join(snomed_dir, 'snomed_active_concepts.txt')
semgroups_path = os.path.join(snomed_dir, 'SemGroups_2018.txt')
# this file is downloaded from https://lhncbc.nlm.nih.gov/ii/tools/MetaMap/Docs/SemGroups_2018.txt

# get relationa, sem types, and sem groups
...
# We want to include these groups: ANAT, CHEM, CONC, DEVI, DISO, PHEN, PHYS, PROC, LIVB
# as these are present in mimic dataset
# And exclude semantic types, which are neither in mimic or icd9:
exclude_types = ['Manufactured Object', 'Geographic Area', 'Professional or Occupational Group', 'Reptile',
                 'Population Group', 'Experimental Model of Disease', 'Professional Society',
                 'Daily or Recreational Activity', 'Amphibian', 'Substance', 'Cell', 'Vertebrate',
                 'Biomedical Occupation or Discipline', 'Amino Acid Sequence', 'Fungus', 'Human', 'Behavior',
                 'Activity', 'Gene or Genome', 'Bird', 'Regulation or Law', 'Organization', 'Machine Activity',
                 'Molecular Function', 'Embryonic Structure', 'Eukaryote', 'Molecular Sequence', 'Virus', 'Archaeon',
                 'Individual Behavior', 'Occupational Activity', 'Nucleotide Sequence', 'Mammal', 'Fish',
                 'Biomedical or Dental Material', 'Self-help or Relief Organization', 'Cell Function',
                 'Chemical Viewed Functionally', 'Chemical Viewed Structurally', 'Governmental or Regulatory Activity',
...
# include the otherwise removed CONC, PHEN -- too broad and not so useful in original script
# these are SG in mimic dataset TODO: possible experiment with and without!
